  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="ml-16 md:ml-20 min-h-screen flex flex-col">
        <Header title={activeTab === 'dashboard' ? 'Dashboard' : activeTab === 'events' ? 'Eventos' : activeTab === 'products' ? 'Produtos' : activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} />
        
        <main className="flex-1 overflow-y-auto">
          {activeTab === 'dashboard' && <DashboardView />}
          {activeTab === 'events' && <EventsView />}
          {activeTab === 'products' && <ProductsView />}
          {activeTab !== 'dashboard' && activeTab !== 'events' && activeTab !== 'products' && (
